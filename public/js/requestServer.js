...
function GenerarAES(_texto, _tipo) {
    ...
    var fechaansi = new Date().toISOString().slice(0, -14).replace(/\-/g, "");
    var ivfecha = fechaansi + fechaansi;
    // var iv = CryptoJS.enc.Utf8.parse(ivfecha);
    var iv = CryptoJS.enc.Utf8.parse('1998102720190705');
    if (_tipo == 1) {
...
class RequestServer {
    ...
    constructor(url, data) {
        ...
        //            }
        //        });
        //        if (!resp.ok) throw 'Error al ejecutar la petición';
        //        let [e, d] = (await resp.json()).d.split(this.separator);
        //        if (e === '1') {
        //            resolve(d);
        //        } else {
        //            reject(d);
    ...
    static exec(procedure, parameters, withUserId = false, withCustomerId = false, saveLocal = false) {
        //console.log("procedure", procedure)
        //console.log("parameters", parameters)

        return new Promise((resolve, reject) => {

            const keyIndexDB = GenerarAES(procedure + '|' + (typeof(parameters) == 'object' ? JSON.stringify(parameters) : parameters), 1);
            const bd = new DataBase('Respuestas', 'urlRequest');
            const getData = () => {

                try {
                    const frm = new FormData();
                    // frm.append("p1", Util.generateAES(procedure, 1));
                    frm.append("p1", procedure);
                    if (withUserId) frm.append('with_user_id', true);
                    if (withCustomerId) frm.append('with_customer_id', true);
                    let data = '';
                    let onlyParametres = '';
                    if (Boolean(parameters)) {
                        for (var key in parameters) {
                            data += '~' + key + '¬' + parameters[key];
                            onlyParametres += ',' + parameters[key]
                        }
                    }
                    if (data.length > 0) data = data.substring(1);
                    // frm.append("p2", Util.generateAES(data, 1));
                    frm.append("p2", onlyParametres.substring(1));

                    let url = "execProcedueSQL";
                    if (Boolean(hdfRaiz) && Boolean(hdfRaiz.value))
                        url = hdfRaiz.value + '/api/execProcedueSQL';
                    ...
                    xhr.onreadystatechange = function(event) {
                        if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
                            try {
                                // console.log(Util.generateAES(decodeURIComponent(escape(window.atob(xhr.responseText))), 2));
                                // debugger
                                // let [e, d] = Util.generateAES(JSON.stringify(xhr.responseText), 2).split('¯*¯');
                                // console.log(e, d);
                                let [e, d] = xhr.responseText.split('¯*¯');
                                switch (e) {
                                    case code.json:
                                        resolve(d);
                                        break;
                                    case code.success:
                                        const obj = {};
                                        if (Boolean(d)) {
                                            const results = d.split('^');
                                            results.forEach(r => {
                                                const [_data, _parse, ..._subParse] = r.split('¬');

                                                const [key, rowSep, colSep, ...props] = _parse.split(';');
                                                obj[key] = parseObject(_data, rowSep, colSep, props);

                                                if (Boolean(_subParse) && _subParse.length > 0) {
                                                    _subParse.forEach(sp => {
                                                        const [key_sp, rowSep_sp, colSep_sp, ...props_sp] = sp.split(';');
                                                        obj[key].forEach(obj_child => {
                                                            obj_child[key_sp] = parseObject(obj_child[key_sp], rowSep_sp, colSep_sp, props_sp);
                                                        });
                                                    });
                                                }
                                            });
                                        ...
                                        if (saveLocal) {
                                            // const objBD = { 'urlRequest': keyIndexDB, 'respuesta': GenerarAES(JSON.stringify(obj), 1) };
                                            const objBD = { 'urlRequest': keyIndexDB, 'respuesta': JSON.stringify(obj) };
                                            bd.upsert(objBD);
                                        }
                                        obj.complete = true;
                                        resolve(obj);
            ...
            if (!saveLocal) {
                ...
                bd.get(keyIndexDB).then(objRpta => {
                    if (Boolean(objRpta)) {
                        // resolve(JSON.parse(GenerarAES(objRpta.respuesta, 2)));
                        resolve(JSON.parse(objRpta.respuesta));
                    } else {
                        getData();
                    }
                });
