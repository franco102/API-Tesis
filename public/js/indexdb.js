...
export default class DataBase {
    ...
    _version;
    _collections = [];
    constructor(collectionName, keyPath) {
        if (Boolean(DataBase.instancia)) {
            DataBase.instancia._collectionName = collectionName;
            DataBase.instancia._keyPath = keyPath;
            DataBase.instancia.updateCollections();
            //DataBase.instancia.init();
    ...
    open() {
        //console.log('open', this._version);
        return self.indexedDB.open(this._name, this._version);
    }
    updateCollections() {
        const exists = this._collections.some(c => c.name === this._collectionName);
        //console.log('update-collections', exists, this._collectionName);
        if (!exists) {
            this._collections.push({
                name: this._collectionName,
                keyPath: this._keyPath
            });
            this._version = this._collections.length;
            //console.log('new collection', this._version, this._collections);
            this.init();
        }
    }
    onUpgradeNeeded(e) {
        //console.log('onUpgradeNeeded', this._collectionName);
        let db = e.target.result;
        db.createObjectStore(
            this._collectionName, { keyPath: this._keyPath }
        );
    }
    get(key) {
        return new Promise((resolve, reject) => {
            if (Boolean(key)) {
                ...
                db.onsuccess = e => {
                    ...
                    let transaction = dataBase.transaction(this._collectionName);
                    let request = transaction.objectStore(this._collectionName).get(key);
                    request.onerror = s => {
                        reject(s.target.error);
                    }
                    request.onsuccess = s => {
    ...
    getAll() {
        return new Promise((resolve, reject) => {
            ...
            db.onsuccess = e => {
                let dataBase = e.target.result;
                let transaction = dataBase.transaction(this._collectionName);
                let request = transaction.objectStore(this._collectionName).getAll();
    ...
    add(obj) {
        return new Promise((resolve, reject) => {
            if (Boolean(obj)) {
                ...
                db.onsuccess = e => {
                    let dataBase = e.target.result;
                    let transaction = dataBase.transaction(this._collectionName, 'readwrite');
                    transaction.onerror = e => reject(e.target.error);
                    transaction.oncomplete = e => resolve(e);
                    transaction.objectStore(this._collectionName).add(obj);
                }
                db.onerror = e => {
                    reject('data-base onerror', e);
                }
            }
        });
    }
            ...
            try {
                if (Boolean(obj)) {
                    ...
                    db.onsuccess = e => {
                        ...
                        const transaction = dataBase.transaction(this._collectionName, 'readwrite');
                        transaction.onerror = t => reject(t.target.error);
                        transaction.oncomplete = t => resolve(t);
                        const request = transaction.objectStore(this._collectionName).put(obj);
                        request.onsuccess = r => resolve(r.target.result);
                        request.onerror = r => reject(r.target.error);
                        dataBase.close();
                    }
                    db.onerror = e => {
                        reject('data-base onerror', e);
                ...
                reject('upsert error', e);
            }
        });
    }
    update(key, obj) {
        return new Promise((resolve, reject) => {
            if (Boolean(key) && Boolean(obj)) {
                let db = this.open();
                db.onsuccess = e => {
                    let dataBase = e.target.result;
                    let transaction = dataBase.transaction(this._collectionName, 'readwrite');
                    transaction.onerror = e => reject(e.target.error);
                    transaction.oncomplete = e => resolve(e);
                    let request = transaction.objectStore(this._collectionName).put(obj);
                    request.onsuccess = e => resolve(e.target.result);
                    request.onerror = e => reject(e.target.error);
                }
                db.onerror = e => {
        ...
        return new Promise((resolve, reject) => {
            if (Boolean(key)) {
                ...
                db.onsuccess = e => {
                    let dataBase = e.target.result;
                    let transaction = dataBase.transaction(this._collectionName, 'readwrite');
                    let request = transaction.objectStore(this._collectionName).delete(key);
                    request.onerror = e => reject(e.target.error);
    ...
    clear() {
        return new Promise((resolve, reject) => {
            ...
            db.onsuccess = e => {
                let dataBase = e.target.result;
                let transaction = dataBase.transaction(this._collectionName, 'readwrite');
                let request = transaction.objectStore(this._collectionName).clear();
                request.onerror = e => reject(e.target.error);
                request.onsuccess = e => resolve();
            }
        });
    ...
    count() {
        return new Promise((resolve, reject) => {
            ...
            db.onsuccess = e => {
                let dataBase = e.target.result;
                let transaction = dataBase.transaction(this._collectionName, 'readwrite');
                let request = transaction.objectStore(this._collectionName).count();
                request.onerror = e => reject(e.target.error);
                request.onsuccess = e => resolve(e.target.result);
            }
        });
